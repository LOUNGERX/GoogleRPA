...
def save_to_word(content, folder_name, target_dir):
    doc = Document()
    doc.add_heading(f'临床病历摘要 - {folder_name}', 0)
    doc.add_paragraph(content)
    safe_name = re.sub(r'[\\/*?:"<>|]', "_", folder_name)
    save_path = os.path.join(target_dir, f"{safe_name}_病历摘要.docx")
    doc.save(save_path)
    return save_path
...
def run_notebooklm_final_fix():
    ...
    with sync_playwright() as p:
        ...
        for folder_name, file_paths in folder_groups.items():
            # 创建结果子文件夹
            target_save_dir = os.path.join(OUTPUT_FOLDER, folder_name)
            ...
            try:
                ...
                if target_locator.count() > 0:
                    ...
                    path = save_to_word(final_text, folder_name, target_save_dir)
                    print(f"🎉 处理成功: {path}")

                    # --- 新增功能：生成并下载演示文稿与视频概览 ---
                    
                    def capture_and_download(label, extension):
                        print(f"🎨 启动{label}生成...")
                        # 记录生成前的条目数（Studio面板中的列表）
                        item_selector = "artifact-library-item, .studio-panel-list-item, [role='listitem']"
                        count_before = page.locator(item_selector).count()
                        
                        # 点击生成按钮
                        gen_btn = page.locator(f"div[role='button'][aria-label='{label}']").last
                        gen_btn.click(force=True)
                        
                        # 等待生成完成（条目数增加且“正在生成”提示消失）
                        for _ in range(300):
                            if page.locator(item_selector).count() > count_before and not page.locator("text=正在生成").is_visible():
                                break
                            time.sleep(5)
                        
                        print("🕒 成果已就绪，触发 30 秒安全等待...")
                        time.sleep(30)

                        try:
                            # 进入详情页
                            page.locator(item_selector).first.click()
                            time.sleep(5)
                            
                            # 点击详情页下载按钮
                            with page.expect_download(timeout=60000) as download_info:
                                page.locator("button[aria-label='下载']").first.click()
                            
                            save_path = os.path.join(target_save_dir, f"{folder_name}_{label}{extension}")
                            download_info.value.save_as(save_path)
                            print(f"✅ {label}下载完成")
                            
                            # 核心修复：点击“折叠”按钮退出详情页，不再使用 Esc
                            # 根据截图，按钮图标名为 collapse_content
                            page.locator("button:has(mat-icon:text('collapse_content'))").first.click()
                            time.sleep(2)
                            
                        except Exception as e:
                            print(f"❌ {label}下载流程出错: {e}")
                            # 尝试关闭详情页以防卡死
                            try: page.locator("button:has(mat-icon:text('collapse_content'))").first.click()
                            except: pass

                    # 顺序执行任务
                    capture_and_download("演示文稿", ".pdf")
                    capture_and_download("视频概览", ".mp4")
                ...
                print(f"❌ 错误: {e}")
                page.screenshot(path=os.path.join(target_save_dir, "fail_screenshot.png"))
